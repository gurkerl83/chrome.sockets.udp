...
- (ChromeSocketsUdpSocket*)initWithId:(NSUInteger)theSocketId plugin:(ChromeSocketsUdp*)thePlugin properties:(NSDictionary*)theProperties
{
    ...
    if (self) {
        ...
        _socket = [[MTGCDAsyncUdpSocket alloc] initWithDelegate:self delegateQueue:dispatch_get_main_queue()];
...
- (void)udpSocket:(MTGCDAsyncUdpSocket *)sock didReceiveData:(NSData *)data fromAddress:(NSData *)address withFilterContext:(id)filterContext
{
    VERBOSE_LOG(@"udbSocket:didReceiveData socketId: %u", _socketId);
    
    [_plugin fireReceiveEventsWithSocketId:_socketId data:data address:[MTGCDAsyncUdpSocket hostFromAddress:address] port:[MTGCDAsyncUdpSocket portFromAddress:address]];
}
